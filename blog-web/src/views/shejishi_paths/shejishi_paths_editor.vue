        ...
        <div class="editor-main">
            <el-form :model="articleForm" :rules="rules" ref="articleForm" label-position="top" size="small">
                ...
                <div class="editor-sidebar">
                    <div class="sidebar-section" v-if="treeSelectOptions.length>0">
                        ...
                        <el-form-item prop="menuId">
                            <treeselect
                                v-model="articleForm.menuId"
                                :options="treeSelectOptions"
                                :normalizer="normalizer"
                                placeholder="选择上级文章目录"
                                clearable
                            />
                        </el-form-item>
                    </div>
...
import { createArticleByAgiApi, updateArticleApi, getArticleInfoApi } from '@/api/article'
import { getTagsApi,getCategoriesApi } from '@/api/tags'
import { getDictDataApi } from '@/api/dict'
import { getMenuListApi} from '@/api/aijsqContent/aijsqMenu'
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                ...
                menuId: [
                    { required: true, message: '选择上级文章目录', trigger: 'change' }
                ],
                tags: [
    ...
    created() {
        this.articleForm.type= this.$route.params.type
      ...
      try {
        ...
        submitArticle() {
            ...
            this.$refs.articleForm.validate(valid => {
                ...
                // let strName= this.articleForm.category
                // if(strName != null || strName != ''){
                //     this.articleForm.categoryId = strName.id
                //     this.articleForm.categoryName = strName.name
                // }
                const api = this.articleForm.id ? updateArticleApi : createArticleByAgiApi
                api(this.articleForm).then(res => {
                    this.$message.success('保存成功')
                    this.$router.push('/shejishi_paths/B5bDcbGhAnRvzWkvF8PcZgk9KsS970UJJiVjYqyJjpd9jx')
                }).catch(err => {
